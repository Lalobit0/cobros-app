...
export default function App() {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      const iNombre = hdrs.findIndex(h => String(h).toUpperCase().includes("NOMBRE"));
      const iCuenta = hdrs.findIndex(h => String(h).toUpperCase().includes("CUENTA") && !String(h).toUpperCase().includes("CUENTA2"));
      const iPrecio = hdrs.findIndex(h => String(h).toUpperCase().includes("PRECIO"));
      const iCol2   = hdrs.findIndex(h => String(h).toUpperCase().includes("COLUMNA2"));
      const iNotas  = hdrs.findIndex(h => String(h).toUpperCase().includes("NOTAS"));
  ...
  const filtrados = useMemo(() => {
    ...
    return clientes
      .filter(c => {
        ...
        if (filtro === "hoy")    return ok && c.dias === 0;
        if (filtro === "semana") return ok && c.dias !== null && c.dias >= 0 && c.dias <= 7;
  ...
  function notificar(c) {
    const diasTxt = c.dias === 0 ? "¡HOY!" : c.dias === null ? "próximamente" : `en ${c.dias} días`;
  ...
  const urgentes = clientes.filter(c => c.dias !== null && c.dias >= 0 && c.dias <= 7).length;
